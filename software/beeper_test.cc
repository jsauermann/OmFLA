...
init_hardware()
{
   ...
   PORTA = pullup_A;
   PORTB = pullup_B | B_TCM_POWER;   // turn off TCM 310 (if installed)
   PORTD = pullup_D;
...
main(int, char *[])
{
   ...
   // beeper is connected to Vcc, so set_pin() tuns it OFF!
   //
   set_pin(D, BEEPER);
   clr_pin(D, LED_RED);
   set_pin(B, LED_GREEN);
         ...
         for (uint8_t j = 0; j < 6; ++j)
             {
               switch(j)
                  {
                     case 0: clr_pin(D, BEEPER);      break;
                     case 1: set_pin(D, BEEPER);      break;
                     case 2: set_pin(D, LED_RED);     break;
                     case 3: clr_pin(D, LED_RED);     break;
                     case 4: set_pin(B, LED_GREEN);   break;
                     case 5: clr_pin(B, LED_GREEN);   break;
                  }

//       for (int32_t wait = 200; wait > 0;)   wait -= sleep_ms(wait);
         for (int32_t wait = 1000; wait > 0; wait -= 20)   _delay_ms(20);
             }
       }
}
//-----------------------------------------------------------------------------
